...
3. the method has two arguments, both exported (or builtin) types.（方法只有2个参数，均为导出类型或内置类型）
4. the method’s second argument is a pointer.（方法的第二个参数必须是指针）
5. the method has return type error.（方法返回值必须是error类型）
更直观一些：
...
// registerMethods 过滤出了符合上述签名规则的方法
func (s *service) registerMethods() {
	...
	for i := range s.typ.NumMethod() {
		...
		mType := method.Type // eg: func(*main.FooService, main.Args, *int) error
		if mType.NumIn() != 3 || mType.NumOut() != 1 { // 参数是3个（包含receiver），返回值是1个
			continue
		}
		if mType.Out(0) != reflect.TypeOf((*error)(nil)).Elem() { // 返回值类型必须是error
			continue
		}
